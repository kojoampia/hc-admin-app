...
type RestOf<T extends IPatient | NewPatient> = Omit<T, 'joinedOn' | 'lastActiveOn'> & {
...
export class PatientsService {
  ...
  protected convertValueFromServer(restPatient: RestPatient): IPatient {
    return {
      ...restPatient,
      joinedOn: restPatient.joinedOn ? dayjs(restPatient.joinedOn) : undefined,
      lastActiveOn: restPatient.lastActiveOn ? dayjs(restPatient.lastActiveOn) : undefined,
    };
  }
}
...
export class PatientService extends PatientsService {
  ...
  protected convertValueFromClient<T extends IPatient | NewPatient | PartialUpdatePatient>(patient: T): RestOf<T> {
    return {
      ...patient,
      joinedOn: patient.joinedOn?.format(DATE_FORMAT) ?? null,
      lastActiveOn: patient.lastActiveOn?.format(DATE_FORMAT) ?? null,
    };
  }

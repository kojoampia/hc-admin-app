...
export class ShiftAssignmentsService {
  ...
  protected readonly resourceUrl = this.applicationConfigService.getEndpointFor('api/shift-assignments');
...
export class ShiftAssignmentService extends ShiftAssignmentsService {
  ...
  find(id: number): Observable<IShiftAssignment> {
    return this.http
  ...
  getShiftAssignmentIdentifier(shiftAssignment: Pick<IShiftAssignment, 'id'>): number {

...
import shiftAssignmentResolve from './shift-assignment-routing-resolve.service';

describe('ShiftAssignment routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IShiftAssignment returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          shiftAssignmentResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(shiftAssignmentResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);

...
type RestOf<T extends ITask | NewTask> = Omit<T, 'dueOn' | 'createdAt'> & {
  dueOn?: string | null;
...
export class TasksService {
  ...
  protected readonly resourceUrl = this.applicationConfigService.getEndpointFor('api/tasks', ADMIN_SERVICE);

  protected convertValueFromServer(restTask: RestTask): ITask {
    return {
      ...restTask,
      dueOn: restTask.dueOn ? dayjs(restTask.dueOn) : undefined,
...
export class TaskService extends TasksService {
  ...
  protected convertValueFromClient<T extends ITask | NewTask | PartialUpdateTask>(task: T): RestOf<T> {
    return {
      ...task,
      dueOn: task.dueOn?.format(DATE_FORMAT) ?? null,

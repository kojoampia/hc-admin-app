...
- **Web storage has one owner: `core/auth/state-storage.service.ts`.** Reach a key through it rather than touching `localStorage` or `sessionStorage` directly. The point of the rule is **one reader per key**, not tidiness: a second copy of a read means a key rename lands in the service, every test of the service passes, and the other caller silently reads a key nobody writes any more. `core/util/message-stream.service.ts` was exactly that until 2026-09-18 — it reimplemented the auth-token read character-for-character, same two stores and same fallback order — and it injects the service now. **`layouts/shell-state.service.ts:60,68` is a named exception and is permitted rather than tolerated**: its sidebar-rail key has a single reader and a single writer, both in that one file, so the rename hazard cannot arise; and its access is `try`/`catch`-wrapped to degrade to "expanded, and do not remember" where storage throws rather than returns null (Safari's private mode, a browser blocking site data), which `StateStorageService`'s unguarded accessors do not do. **Do not add a third.** Nothing enforces this — the sweep is `grep -rn 'localStorage\|sessionStorage' --include='*.ts' src/main/webapp/app`, and outside `*.spec.ts` it should name those two files and no others. ⚠ **That sweep reads `.ts` under `app/` only, so it is blind to any non-`.ts` asset and to anything outside `app/` — and it misses a third reader of the auth key today.** `swagger-ui/index.html:29` reads `'abf-authenticationToken'` from both stores as a **hardcoded string literal**, with an unguarded `JSON.parse` after it, and `angular.json` ships `src/main/webapp/swagger-ui`, so it is live on `/admin/docs`. Tracked separately; do not fold a fix for it into an unrelated change. **A clean sweep here means two files, not two readers** — and a key written as a literal is invisible to a search for the constant that names it, which is the rename hazard in its worst form.
